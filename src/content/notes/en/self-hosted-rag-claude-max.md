...
**OpenKB** is the index. It takes the documents I care about (papers, contracts, reference material, personal notes) and compiles them into a queryable wiki with cross-document links. It exposes itself to Claude Code as a stdio MCP server, which means a Claude session can ask it questions through a typed tool interface. The compile step does its own LLM call internally; I have it set to Claude Opus 4.8, because the linking is better than at smaller sizes and the speed difference does not matter for a background job.
